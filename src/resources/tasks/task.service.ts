...
(function () {
   ...
   */
  const getTasks = async (boardId: string | null) => {
    const tasks: ITask[] = await repo.getAll();
   ...
   */
  const getTaskById = async (baordId: string, taskId: string) => {
   ...
   */
  const putData = async (oParams: ITask) => {
   ...
   */
  const addData = async (oParams: ITask) => {
   ...
   */
  const deleteTasks = async (taskId: string) => repo.deleteTask(taskId);

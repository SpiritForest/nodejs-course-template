...
(function () {
   ...
   */
  const findIndexById = (id: string) => TASKS.findIndex(data => data.id === id)
   ...
   */
  const addData = async (task: ITask) => {
    TASKS.push(task);
    return task;
  };
   ...
   */
  const replaceData = async (index: number, data: ITask) => {
   ...
   */
  const deleteTaskByBoard = async (boardId: string) => {
